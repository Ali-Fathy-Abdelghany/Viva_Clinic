...
const deleteSpecialty = asyncHandler(async (req, res) => {
    ...
    if (doctorsCount > 0) {
        throw new AppError(
            "Cannot delete specialty. It is assigned to doctors.",
            StatusCodes.BAD_REQUEST
        );
    }
...
const updateDoctorWorkingHours = asyncHandler(async (req, res) => {
    ...
    await DoctorWorkingHours.destroy({
        where: { DoctorID: id, DayOfWeek: { [Op.in]: daysToUpdate } },
    });
...
const createAdmin = asyncHandler(async (req, res) => {
    ...
    const admin = await User.findByPk(newAdmin.UserID, {
        attributes: { exclude: ["PasswordHash"] },
    });
    res.status(StatusCodes.CREATED).json({
        success: true,
