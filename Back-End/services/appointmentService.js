...
const checkWorkingHours = async (
    ...
    const workStart = workingHours.StartTime.slice(0, 5);
    const workEnd = workingHours.EndTime.slice(0, 5);
        ...
        const conflict = await Appointment.findOne({
            where: {
                ...
                [Op.or]: [
                    {
                        StartTime: {
                            [Op.lt]: slotEnd.toTimeString().slice(0, 8),
                        },
                        EndTime: {
                            [Op.gt]: slotStart.toTimeString().slice(0, 8),
                        },
                    },
                ],
                Status: { [Op.notIn]: ["Cancelled"] },
