...
function updateView() {
    ...
    if (state.filterDate) {
        ...
        if (dateFilterText) {
            dateFilterText.textContent = dateObj.toLocaleDateString("en-GB", {
                day: "numeric",
                month: "short",
                year: "numeric",
            });
...
function renderTable(data) {
    ...
    data.forEach((appt) => {
        ...
        row.className =
            "border-b border-gray-50 hover:bg-gray-50 transition group";
        row.id = `row-${appt.id}`;

        let statusClasses = "";
        if (appt.status === "Booked")
            statusClasses = "border-blue-200 text-[#4B7BEC] bg-blue-50";
        else if (appt.status === "Cancelled")
            statusClasses = "border-red-200 text-[#FF6B6B] bg-white";
        else if (appt.status === "Completed")
            statusClasses =
                "border-green-200 text-[#26DE81] bg-white uppercase";
        else if (appt.status === "Rescheduled")
            statusClasses =
                "border-yellow-300 text-yellow-600 bg-yellow-50 capitalize";
...
function renderCalendar() {
    ...
    for (let day = 1; day <= daysInMonth; day++) {
        ...
        let baseClasses =
            "py-2 flex justify-center items-center relative cursor-pointer hover:bg-gray-50 rounded-lg transition";
        if (isSelected)
            baseClasses +=
                " ring-2 ring-[#3A8F86] ring-offset-1 bg-teal-50 font-bold";
        dayDiv.className = baseClasses;

        const apptsOnDay = state.data.filter(
            (a) =>
        ...
        if (apptsOnDay.length > 0) {
            ...
            else if (latestAppt.status === "Cancelled")
                bgClass = "bg-[#FF6B6B]";
            else if (latestAppt.status === "Rescheduled")
                bgClass = "bg-[#F7B731]";
